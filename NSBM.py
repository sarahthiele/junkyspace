...
parser.add_argument("--event", default='India',
                    help='choose from: India, Russia, FTG15', type=str)
...
# Specify working directory where sims will be stored
path = str(args.path)
...
altref = 300
deorbit_R = 200.  # remove fragments when they go below 200 km
...
dAltCo = (altCoMax-altCoMin)/NALT  # altitude bin width
dThetaCo = np.pi/NTHETA  # colatitude bin width
...
if event == 'India':
    ...
    inc = 82.6 * np.pi / 180
    omega= 0. * np.pi / 180
elif event == 'FTG15':  # These need to be double checked !! 
    mkill = 64.
    vkill = 5.2e3
    mtarget = 1e3
    Q = 1250 + REkm
...
velvec = vel_dis_NBM(mtarget, mkill, vkill, vtarget, rtarget, nbins, Lc_min,
                     Lc_max, KEkill, numsample, makev=True)

N_tot, L_mids, nums, mfrags, vfrags_all, vfrags, vfrags_total, eccs, SMA,\
AMfrags_all, AMfrags, Lcvals_all, Lcvals = velvec
...
ax[0].text(0.7, 0.9, r'N$_{\rm{TOT}}$='+str(N_tot), fontsize=18, 
...
tlabel = "Mtarget={:.3} ton, Vtarget={:.3} km/s, "\
         "Mkill={} kg, Kill Energy={:.2} GJ, "\
         "Intercept Height={} km".format(mtarget*1./1000, 
                                         np.round(mag(vtarget),1)/1e3,
                                         mkill, KEkill/1e9, r-REkm)
ax[0].text(0.5, 1.075, tlabel, fontsize=22, transform=ax[0].transAxes)

plt.savefig('{}/init_dis_{}_{}_{}_{}.png'.format(data_path, Lc_min, numsample, 
                                                 KEkill/1e9, maxtime), 
            bbox_inches='tight')
...
# fragments that are in elliptical orbits and have deorbited:
init_deorbit = (SMA*(1-eccs)/1000 <= REkm + deorbit_R)&(eccs<1.0) 

# fragments that have been ejected into hyperbolic orbit:
ejected = eccs >= 1.0  

# define data frame of fragments that deorbited immediately:
data1 = np.array([np.linalg.norm(vfrags[init_deorbit], axis=1), 
                  AMfrags[init_deorbit], SMA[init_deorbit], 
                  eccs[init_deorbit], Lcvals[init_deorbit]])
df1 = pd.DataFrame(data1.T, columns=['vkick', 'AM', 'SMA', 'ecc', 'Lc'])
df1['t_deorbit'] = 0.

# define data frame of fragments that are ejected from orbitL
...
# initialize data for the sim. Ejected/deorbited fragments don't contribute 
...
plt.savefig('{}/init_gabbard_kept_LEO_{}_{}_{}_{}.png'.format(data_path, Lc_min,
                                                              numsample, 
                                                              KEkill/1e9,
                                                              maxtime),
            bbox_inches='tight')
...
plt.subplots_adjust(wspace=0.35)
tlabel = "Mtarget={:.3} ton, Vtarget={:.3} km/s, Mkill={} kg,\nKill "\
         "Energy={:.2} GJ, Intercept Height={} km".format(mtarget*1./1000, 
                                                          np.round(mag(vtarget),
                                                                   1)/1e3, 
                                                          mkill, KEkill/1e9, 
                                                          r-REkm)
ax[0].text(0.2, 1.075, tlabel, fontsize=20, transform=ax[0].transAxes)

plt.savefig('{}/kept_init_dis_{}_{}_{}_{}.png'.format(data_path, Lc_min, 
...
#============================================================================
# add position variations of order 10 metres to all fragments and
...
for i in range(len(vfrags)):
    sim.add(m=0., vx=vfrags_total[i, 0], vy=vfrags_total[i,1], 
            vz=vfrags_total[i,2], x=x[i], y=y[i], z=z[i], 
            hash='{}'.format(i+1))
...
data3 = np.array([np.linalg.norm(vfrags, axis=1), AMfrags, SMA, eccs, Lcvals, 
                  deorbit_times, colprobs])
df3 = pd.DataFrame(data3.T, columns=['vkick', 'AM', 'SMA', 'ecc', 'Lc',
                                     't_deorbit', 'colprob'])

datadf = df.append(df3)
datadf = datadf.sort_values('t_deorbit')
datadf.to_hdf('{}/data_{}_{}_{}_{}.hdf'.format(data_path, Lc_min, numsample, 
                                               KEkill/1e9, maxtime), key='data')
dataother = pd.DataFrame(np.array([nancatch]), columns=['nancatch'])
dataother.to_hdf('{}/data_{}_{}_{}_{}.hdf'.format(data_path, Lc_min, numsample,
                                                  KEkill/1e9, maxtime),
                 key='nancatch')
...
plt.title('{:.4}% deorbited after {} years'.format(cumsum[-1]/NFOLLOW*100, 
                                                   maxtime), fontsize=16)
plt.savefig('{}/deorbit_times_{}_{}_{}_{}.png'.format(data_path, Lc_min, 
                                                      numsample, KEkill/1e9, 
                                                      maxtime), 
            bbox_inches='tight')
...
plt.scatter(np.log10(datadf.AM.values[flageject]),
            datadf.vkick.values[flageject]/1000,
            c='xkcd:cyan', label=r'ejected fragments')
plt.scatter(np.log10(datadf.AM.values[flaghi]),
            datadf.vkick.values[flaghi]/1000,
            c='r', label=r't$_{\rm{deorbit}} \geq$'+' {}yrs'.format(maxtime))
plt.scatter(np.log10(datadf.AM.values[flaglo]), 
            datadf.vkick.values[flaglo]/1000,
...
ax.tick_params(labelsize=14)
plt.savefig('{}/AM_vkicks_times_{}_{}_{}_{}.png'.format(data_path, Lc_min, 
                                                        numsample, KEkill/1e9, 
                                                        maxtime),
            bbox_inches='tight')
